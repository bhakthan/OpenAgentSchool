...
import { Target, Download, ArrowsOutSimple, ArrowsInSimple, ArrowClockwise, Sun, Moon, ArrowLeft, Eye, EyeSlash } from "@phosphor-icons/react"
...
export default function AIProductFrameworkPage() {
  ...
  useEffect(() => {
    ...
    const verticalSpacing = layoutMode === 'compact' ? 26 : 34
    const horizontalSpacing = layoutMode === 'compact' ? 95 : 110
    ...
    const node = g.selectAll('.node')
      ...
      .on('click', function(event, d: any) {
        ...
        // If clicking a pillar node, update details panel
        if (d.depth === 1 && d.data.caseStudy) {
    ...
    node.append('circle')
      ...
      .style('fill', (d: any) => {
        if (d.depth === 0) return isDark ? '#e74c3c' : '#e74c3c'
    ...
    node.append('text')
      ...
      .text((d: any) => {
        const maxLen = d.depth === 0 ? 30 : (d.depth === 1 ? 25 : 40)
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto px-4 py-6">
          ...
          <div className="mt-4">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h1 className="text-3xl font-bold">AI Product Framework</h1>
            </div>
            <p className="text-muted-foreground">
              8 Pillars for Trust-Centric Design: A practical, actionable framework for building trustworthy AI products that drive adoption and reduce risk
            </p>
          </div>
      ...
      {/* Main Content */}
      <div className="container mx-auto px-4 py-6">
        <div className={`grid gap-6 ${showDetails ? 'lg:grid-cols-[1fr,400px]' : 'grid-cols-1'}`}>
          {/* Tree Visualization */}
          <Card>
            <CardHeader>
              <CardTitle>Framework Hierarchy</CardTitle>
              <CardDescription>
                Click nodes to expand/collapse. Click pillar names to view details.
              </CardDescription>
            </CardHeader>
            <CardContent>
          ...
          {showDetails && selectedPillar && (
            <Card className="lg:sticky lg:top-6 h-fit max-h-[800px] overflow-y-auto">
              <CardHeader>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <CardDescription className="mt-1">{selectedPillar.coreRes}</CardDescription>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="space-y-6">
                ...
                {/* Business Impact */}
                <div className="p-4 bg-muted/40 rounded-lg border border-primary/20">
                  <h3 className="font-semibold mb-2">Business Impact</h3>
                  <p className="text-sm text-muted-foreground">{selectedPillar.businessImpact}</p>
                </div>
                ...
                <div className="p-4 bg-card border rounded-lg space-y-3">
                  ...
                  <div className="space-y-3 text-sm">
                    <div>
                      ...
                      <p className="text-muted-foreground">{selectedPillar.caseStudy.action}</p>
                    </div>
                    <div>
                      <div className="font-medium mb-1">Outcome</div>
                      <p className="text-muted-foreground">{selectedPillar.caseStudy.outcome}</p>
                    </div>
                    <div>
                      <div className="font-medium mb-1">Result</div>
                      <p className="text-muted-foreground">{selectedPillar.caseStudy.result}</p>
                    </div>
                  </div>
                </div>
              </CardContent>
        ...
        <div className="mt-12 space-y-6">
          <h2 className="text-2xl font-bold">Complete Framework Overview</h2>
          
          {PILLARS.map((pillar, idx) => (
            <Card key={idx} className="cursor-pointer hover:border-primary/50 transition-colors"
                  onClick={() => { setSelectedPillar(pillar); setShowDetails(true); window.scrollTo({ top: 0, behavior: 'smooth' }); }}>
              <CardHeader>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <CardDescription>{pillar.coreRes}</CardDescription>
                  </div>
                </div>
              </CardHeader>
            </Card>
