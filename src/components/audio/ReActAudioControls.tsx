...
interface ReActAudioControlsProps {
  componentName: string;
  className?: string;
  activeTab?: string;
}

export default function ReActAudioControls({ 
  componentName, 
  className = '',
  activeTab = 'flow-diagram'
}: ReActAudioControlsProps) {
  ...
  const contentTypes = {
    PatternGuide: {
      ...
      label: 'Pattern Guide',
      description: 'Conceptual overview and business applications',
    ...
    CodePractice: {
      ...
      label: 'Code Practice',
      description: 'Implementation examples and hands-on practice',
  ...
  const tabToContentType: Record<string, string> = {
    ...
    'details': 'PatternGuide', 
    'implementation': 'CodePractice'
  };
    ...
    <TooltipProvider>
      <div className={`flex items-center gap-3 flex-wrap ${className}`}>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <SpeakerHigh className="w-4 h-4 text-blue-600 dark:text-blue-400" />
          <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Audio:</span>
          <Badge className="ring-1 bg-[var(--badge-blue-bg)] ring-[var(--badge-blue-ring)] text-[var(--badge-blue-text)] dark:text-[var(--badge-blue-text)] text-xs">
            {filteredContentTypes[currentContentType]?.label || 'ReAct Enhanced'}
          </Badge>
        </div>
        ...
        <div className="flex gap-1 flex-wrap">
          {Object.entries(filteredContentTypes).map(([type, config]) => 
            Object.entries(levelConfig).map(([level, levelConf]) => {
        ...
        {isPlaying && (
          <div className="flex items-center gap-2 p-1 bg-blue-50 dark:bg-blue-950 rounded border border-blue-200 dark:border-blue-800 flex-shrink-0">
