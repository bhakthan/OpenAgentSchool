...
export const failureModes: FailureMode[] = [
  ...
  { patternId: 'hierarchical-document-intelligence', mode: 'context-fragmentation', description: 'Chunking breaks semantic units (tables, diagrams) mid-content, losing relationships.', impact: 'high', detectionSignal: 'Cross-reference questions return incomplete or wrong answers.', remediation: 'Use content-aware chunking that preserves table/diagram/section boundaries.' },
  { patternId: 'hierarchical-document-intelligence', mode: 'synthesis-hallucination', description: 'Hierarchical synthesizer invents relationships not present in source document.', impact: 'high', detectionSignal: 'Engineer rejects synthesized insights; cannot find citations.', remediation: 'Require all synthesis claims cite specific source pages; add human-in-loop verification.' },
  { patternId: 'hierarchical-document-intelligence', mode: 'query-cache-miss', description: 'Query handler re-processes entire document instead of using indexed memory.', impact: 'medium', detectionSignal: 'Response latency >10s for simple cross-reference questions.', remediation: 'Verify vector index populated; check Redis cache hit rate.' },

  // Contextual Onboarding Orchestrator
  { patternId: 'contextual-onboarding-orchestrator', mode: 'context-amnesia', description: 'Memory summarization fails, system forgets employee profile after 5+ turns.', impact: 'high', detectionSignal: 'Employee must re-state role/department in later sessions.', remediation: 'Verify summarization trigger executes; check profile_summary field populated.' },
  { patternId: 'contextual-onboarding-orchestrator', mode: 'misrouted-questions', description: 'Questions consistently routed to wrong specialist agent.', impact: 'medium', detectionSignal: 'HR questions answered by DevOps agent; low user satisfaction.', remediation: 'Review routing prompt clarity; add employee context to classification.' },
  { patternId: 'contextual-onboarding-orchestrator', mode: 'token-waste', description: 'Full conversation history sent on every turn despite summarization capability.', impact: 'medium', detectionSignal: 'Token usage grows linearly with turn count; no compression.', remediation: 'Ensure context_limit parameter set correctly; verify summarization triggers.' }
];
