...
Focus on encouraging continued learning while providing specific, actionable feedback. Recognize effort and thinking process, not just correct answers.${request.expectedInsights?.length ? ` Reference the expected insights to guide deeper exploration.` : ''}`;
  ...
  try {
    ...
    return {
      score: 50,
      feedback: "Thank you for your thoughtful responses. Continue exploring these concepts!",
      suggestions: ["Keep practicing with similar questions", "Try to connect concepts to real-world examples"],
      insights: [],
      strengths: ["Engaged with the learning process"],
      improvements: ["Continue developing deeper analysis skills"]
    };
  }
}
...
Focus on debugging methodology, systematic thinking, and practical problem-solving skills.${request.challengeDetails?.commonMistakes?.length ? ` Check if they avoided common mistakes: ${request.challengeDetails.commonMistakes.join(', ')}.` : ''}${request.challengeDetails?.learningObjectives?.length ? ` Assess progress toward learning objectives: ${request.challengeDetails.learningObjectives.join(', ')}.` : ''}`;
  ...
  try {
    ...
    return {
      score: 50,
      feedback: "Your debugging approach shows good thinking. Keep practicing systematic debugging!",
      suggestions: ["Break down the problem into smaller steps", "Look for patterns in the error messages"],
      insights: [],
      strengths: ["Applied systematic debugging thinking"],
      improvements: ["Continue developing debugging methodology"]
    };
  }
}
...
Provide constructive, educational feedback that encourages learning and improvement.`;
  ...
  try {
    ...
    return {
      score: 50,
      feedback: "Good approach! Continue practicing with these types of challenges.",
      suggestions: ["Try to think through the problem step by step", "Consider different approaches to the solution"],
      insights: [],
      strengths: ["Engaged with the challenge"],
      improvements: ["Continue developing problem-solving skills"]
    };
  }
}
